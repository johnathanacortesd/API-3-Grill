...
- Con IA/PKL, `Tono_IA`, `Tema_IA`, `Subtema_IA` se insertan **después de `Audiencia`**.
  `Contexto analizado` va **última**. `resumen corto` y `revalorización` **no** se exportan.
  El resto de columnas conserva su orden relativo previo.

...
def normalize_brand_mentions(brand: str, aliases: List[str]) -> List[str]:
    """Devuelve solo variantes explícitas de la marca; nunca prefijos genéricos."""
    values = [str(brand or "").strip()] + [str(a or "").strip() for a in (aliases or [])]
    out = []
    for value in values:
        norm = unidecode(value.lower()).strip()
        if norm and norm not in out:
            out.append(norm)
    return out


def build_brand_context(title: str, body: str, brand: str, aliases: List[str]) -> Dict[str, str]:
    """Extrae contexto únicamente cuando existe una mención explícita de la marca."""
...
def subtopic_from_event(text: str, brand: str = "") -> str:
    ...
    brand_words = set(normalize_text_for_matching(brand).split())
    words = [w for w in re.findall(r"[A-Za-zÁÉÍÓÚáéíóúÑñ0-9]+", clean) if normalize_text_for_matching(w) not in brand_words]
    words = [w for w in words if normalize_text_for_matching(w) not in STOPWORDS_ES]
    return " ".join(words[:8]).capitalize() or "Hecho informativo"
...
def _cluster_similar_rows_greedy(rows: List[dict], km: dict, brand_regexes: List[str]) -> Dict[int, int]:
    n = len(rows)
    cluster_map = {}
    clusters_rep = {}
    current_cluster = 0
    
    active_indices = [i for i in range(n) if not rows[i].get("is_duplicate") and rows[i].get("_brand_presence", "present") != "absent"]
...
def canonicalize_subtopics(cluster_results: Dict[int, Tuple[str, str, str]]) -> Dict[int, Tuple[str, str, str]]:
    """Conserva etiquetas; la unificación principal ocurre al agrupar hechos."""
...
def enrich_rows_with_ai(
    ...
    tone_model=None,
    theme_model=None,
    strict_brand_presence: bool = False,
) -> List[dict]:
    ...
    for row in rows:
        if row.get("is_duplicate"):
            ...
            row["Contexto analizado"] = ctx["text"] or "-"
    ...
    for row_idx, cid in cluster_map.items():
        rows[row_idx]["Grupo noticia"] = f"G{cid + 1:04d}"
        rows[row_idx]["Confianza agrupación"] = "Alta" if cid in set(cluster_map.values()) else "Media"
    ...
    for i, row in enumerate(rows):
        ...
        if row.get("_brand_presence") == "absent" and strict_brand_presence:
            row["Tono_IA"] = "No aplica"
            row["Tema_IA"] = "-"
            row["Subtema_IA"] = "-"
            row["Confianza tono"] = "Alta"
            continue
